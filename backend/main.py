...
SYSTEM_PROMPT = (
    ...
    "If the assignee contains ANY of these words — team, group, department, squad, "
...
def _norm_name(text: str) -> str:
    """
    Normalize a group/user name for fuzzy matching.
...
def _find_group_by_name(spoken_name: str) -> tuple[object, dict] | tuple[None, None]:
    """
    Scan all Firestore groups and return (doc, data) for the first one whose
    normalized name matches the normalized spoken_name.
    Returns (None, None) if no match is found.
    """
    target = _norm_name(spoken_name)
    if not target:
        return None, None
    ...
    for gdoc in db.collection("groups").stream():
        gdata = gdoc.to_dict()
        stored = _norm_name(
            gdata.get("group_name_lower") or gdata.get("group_name", "")
        )
        if stored == target:
            print(f"[group-match] '{spoken_name}' → matched group '{gdata.get('group_name')}' (id={gdoc.id})")
            return gdoc, gdata

    print(f"[group-match] '{spoken_name}' → no matching group found")
    return None, None
    """
...
async def voice_command(audio: UploadFile = File(...), x_user_id: Optional[str] = Header(None)):
    """
    Accepts an audio file, transcribes it with Whisper, then routes to either:
      - create_task:    extract + validate assignee + save to Firestore
      - approve_tasks:  bulk-update matching pending tasks to 'approved'
    """
    ...
    try:
        ...
        # ── UPDATE TASK STATUS (covers approve, complete, bulk, specific) ────────
        if command_type in ("update_task_status", "approve_tasks"):
            ...
            assignee_name: Optional[str] = raw_assignee if raw_assignee else None
            ...
            # Resolve current user's name for permission checks
            ...
            # Scope to authenticated user when no assignee named
            if assignee_name is None and current_user_name:
                assignee_name = current_user_name

            # Permission check for specific task updates
        ...
        if command_type == "leave_group":
            ...
            group_name_lower = (task_data.get("group_name") or "").strip().lower()
            if not group_name_lower:
            ...
            if not results:
                raise HTTPException(status_code=404, detail=f"Group '{group_name_lower}' not found.")
            ...
            print(f"Voice: user {x_user_id} left group '{group_name_lower}'")
            return {
                "action":     "leave_group",
                "success":    True,
                "transcript": transcript,
                "group_name": group_data.get("group_name", group_name_lower),
                "group_id":   group_doc.id,
            }
        ...
        if command_type == "remove_member":
            ...
            if not group_name_lower or not member_name_lower:
            ...
            print(f"Voice: owner {x_user_id} removed '{member_name_lower}' from group '{group_name_lower}'")
            return {
        ...
        # ── CREATE GROUP TASK ──────────────────────────────────────────────────
        if command_type == "create_group_task":
            group_name = (task_data.get("group_name") or "").strip().lower()
            if not group_name:
                raise HTTPException(status_code=400, detail="Could not extract group_name from voice command.")

            # Only the group owner may assign group tasks via voice
            g_results = (
                db.collection("groups")
                .where("group_name_lower", "==", group_name)
                .limit(1)
                .get()
            )
            if not g_results:
                raise HTTPException(status_code=404, detail=f"Group '{group_name}' not found.")
            if g_results[0].to_dict().get("created_by") != x_user_id:
                raise HTTPException(
                    status_code=403,
                    detail="Only the group owner can assign tasks to this group.",
                )
        ...
        # ── CREATE TASK ────────────────────────────────────────────────────────
        assignee_name = (task_data.get("assignee_name") or "").strip().lower()
        task_data["assignee_name"] = assignee_name  # normalise before saving + response

        print(f"[voice-command] command_type=create_task, assignee_name='{assignee_name}'")

        # ── NORMALIZE helper: strip everything except a-z and 0-9 ─────────────
        # "front-end team" → "frontendteam"
        # "frontend team"  → "frontendteam"
        # "front end team" → "frontendteam"
        ...
        assignee_norm = _norm(assignee_name)

        # ── GROUP DETECTION: scan all Firestore groups with normalized matching ─
        # This catches exact matches AND variations like "front-end team" vs
        # "frontend team" that exact string equality would miss.
        _matched_group_doc  = None
        _matched_group_data = None

        all_groups = db.collection("groups").stream()
        for _gdoc in all_groups:
            _gdata = _gdoc.to_dict()
            stored_name = (_gdata.get("group_name_lower") or _gdata.get("group_name", "")).lower()
            if _norm(stored_name) == assignee_norm:
                _matched_group_doc  = _gdoc
                _matched_group_data = _gdata
                print(
                    f"[voice-command] LLM misclassified group '{assignee_name}' as user — "
                    f"matched Firestore group '{stored_name}' — auto-correcting to create_group_task"
                )
                break

        if _matched_group_doc is not None:
            # ── Owner check ───────────────────────────────────────────────────
            if _matched_group_data.get("created_by") != x_user_id:
                raise HTTPException(
                    status_code=403,
            ...
            # ── Build group task ──────────────────────────────────────────────
            canonical_group_name = _matched_group_data.get("group_name", assignee_name)
            task_data["group_name"] = canonical_group_name
        ...
        # ── No group matched — validate as a real user ────────────────────────
        print(f"[voice-command] '{assignee_name}' not matched to any group — attempting user lookup")
        try:
            sender_id = validate_assignee(assignee_name)
        except HTTPException:
            # Not a valid user AND not a group — return a clean 400, never 503
            raise HTTPException(
                status_code=400,
                detail=(
                    f"'{assignee_name}' is neither a registered user nor a known group. "
